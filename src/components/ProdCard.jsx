...
function ProdCard() {
  ...
  const toggleWishlist = (e, id) => {
    e.preventDefault(); // Prevents link navigation triggers
    e.stopPropagation();
        ...
        {products && products.map((product) => {
          ...
          return (
            ...
            >
              ...
              {/* FIXED: The main link now strictly wraps the product content, NOT the action buttons */}
              <NavLink to={`/products/${product.id}`} className="block flex-1 flex flex-col">
                ...
                <div className="w-full aspect-square bg-slate-950 rounded-xl border border-slate-900/60 flex items-center justify-center relative overflow-hidden group-hover:border-slate-800/40 transition-colors">
                  ...
                  {/* Heart Toggle Switch (Isolated with absolute and pointer events) */}
                  <button
                    type="button"
              ...
              {/* FIXED: Brought out of the outer product link parent container cleanly */}
              <div className="mt-4">
                <NavLink to='/cart' className="block w-full">
                  <button 
                    onClick={() => dispatch(addToCart(product))} 
                    className="w-full flex items-center justify-center gap-2 bg-slate-900 hover:bg-blue-600 border border-slate-800 hover:border-blue-500 font-bold py-2 rounded-xl text-[11px] tracking-wide text-slate-300 hover:text-white transition-all active:scale-[0.98] group/btn shadow-md shadow-black/10"
                  >
